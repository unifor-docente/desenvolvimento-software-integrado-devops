...
Revisado em 25/09/2026: **70 slides de conteúdo + capa institucional**, com 65 na trilha principal, 5 consultas, 15 diagramas editáveis e seis verificações (18 situações) com gabaritos no slide seguinte.
...
- **Índice de perguntas e respostas.md:** localiza as seis verificações e transcreve os gabaritos.
...
A apresentação principal contém E5 nas posições **186–256**. O identificador de rodapé E5·NN corresponde à página NN + 1 do PDF independente. O guia PowerPoint é uma seleção que mantém esses identificadores.
...
## Ajuste de CI, Delivery e Deployment

A abertura distingue os três fluxos. O novo bloco de automação separa validação, transporte da imagem, aprovação, destino persistente e teste pós-deploy. O laboratório manual está identificado como ensaio local. [Automação comentada e pré-requisitos](../../Práticas/encontro-5-entrega/AUTOMACAO-CD.md). O workflow de referência não foi ativado; a prática formal e os pesos permanecem.
